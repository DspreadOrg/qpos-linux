...
{
    ST_TIME t_systime;
    time_t beijingTime= ntpTm + 8*60*60;  //北京时间和UTC差8小时
...
#define PR_ARRAY_SIZE(arr) sizeof(arr)/(sizeof(arr[0]))
void *ntp_sync_time_process (void *argv)
{
    ...
    {   
        ...
        for ( i=0 ; i<PR_ARRAY_SIZE(ntp_server_list) ; i++ ) {
            ret = ntp_sync_time((char *)ntp_server_list[i]);//def set beijing time
            if ( ret == 0 ) {
                return NULL;
            }
        }
