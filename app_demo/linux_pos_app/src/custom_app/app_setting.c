...
static u32 AnalysisWifiList(pu8 pValue,T_U8_VIEW list[])
{
    ...
    while(cnt<=strlen((char*)pValue))
    {
        ...
        if(pValue[cnt]=='|')
        {
            ...
            OsLog(LOG_DEBUG,"111wifiName[%d]:%s",len,wifiNmae);
        ...
        else if(pValue[cnt]=='}')
        {
            ...
            memset(wifiNmae,0,64);
            memcpy(wifiNmae,list[i].head,len);
            OsLog(LOG_DEBUG,"222wifiName[%d]:%s",len,wifiNmae);
            break;
        }
        memset(wifiNmae,0,64);
        memcpy(wifiNmae,list[i].head,len);
        OsLog(LOG_DEBUG,"333wifiName[%d]:%s",len,wifiNmae);
    }
    return i+1;
}
...
RESEARCH:		
    ...
    if(res==0x00){
        Memset_(uvlist, 0x00, sizeof(uvlist));
...
void  Wifi_Connect()
{
    ...
    {
        Memcpy_(strApSel,wifi_name.head,wifi_name.len);
        Memcpy_(strPassword,wifi_pwd.head,wifi_pwd.len);
    }
    if(g_SettingBuffer)
    {
        free(g_SettingBuffer);
    ...
    if(Rc==RC_SUCCESS)
    {
		...
		// DispWifiConnectionMessage("Connect to Wi-Fi", (char*)strApSel);
    }
    else
    {
