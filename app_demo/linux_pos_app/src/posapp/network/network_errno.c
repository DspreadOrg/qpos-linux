...
const socket_errno_map_t  errno_message_map[] = {
    ...
    {SOCK_ERR_STEP_SSL_INIT,      "SSL初始化失败"    },
    {SOCK_ERR_STEP_SSL_CONNECT ,  "SSL连接错误"      },
    {SOCK_ERR_STEP_SSL_SEND ,     "SSL发送失败"      },
    {SOCK_ERR_STEP_SSL_RECV ,     "SSL接收失败"      },

    {SOCK_ERR_CURL_PARA,          "HTTP:参数错"      },
    {SOCK_ERR_CURL_INIT,          "HTTP:初始化错"    },
