...
PR_INT32 	TransView_nShowUserInputPWD(PR_INT32  nAtLine,
										   PR_INT32  nLines,
										   PR_UINT32  *nTypeOfIM,
										   PR_INT32  nTypeOfIMLen,
										   PR_INT32  nMaxLength,
										   PR_INT32  nMinLength,
										   PR_INT32  nLenOfOutputBuffer, //Enter Buffer length
										   PR_INT8*  pcOutput)
{
	PR_INT32 nRet = 0;
	PR_INT32 nOffset = 0;
	...
	nOffset += 1;
	nRet =  Disp_nUserInputViewEx(DEFAULT_LEFT_MARGIN, DEFAULT_LINE_YPOINT(nAtLine), DEFAULT_PORT_WIDTH, (DEFAULT_HEIGHT_LINE*nLines),
							nMinLength,(nMaxLength + nOffset),nTypeOfIM, nTypeOfIMLen,nLenOfOutputBuffer, (PR_UINT8*)pcOutput, TimeOut, DPORT_DEFAULT_STYLE|DPORT_ENABLE_LANGEN|DPORT_PWD_INPUT);
										...
										PR_INT8* pcOfflinePinOutput)
{
	...
	{
// Query transactions do not need to display the amount
	...
	szInputPin.nPinMode = 0x01; //Fixed 01, offline pin
...
// The length limit for input is: Limited number + 1 decimal point + currency symbol length + 1 space
																	  ...
																	  PR_INT8*  pcOutput)
{
	...
	TransView_vShowLine(1,EM_ALIGN_LEFT, EM_DTYPE_NORMAL,"%s",pszTip);
	PR_UINT32 acIme[3]={DPORT_IME_NUM,DPORT_IME_EN,DPORT_IME_SYMBOL};
	return TransView_nShowUserInputPWD(3,2,acIme,3,MaxLength,MinLength,nLenOfOutputBuffer, pcOutput);
}
