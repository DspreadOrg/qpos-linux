...
lv_obj_t * lv_list_add_btn_new(lv_obj_t * list, const char * SerialNum, const char * txt)
{
    LV_LOG_INFO("begin");
    lv_obj_t * obj = lv_obj_class_create_obj(&lv_list_btn_class, list);
    lv_obj_class_init_obj(obj);
    ...
    if(SerialNum) {
        lv_obj_t * serNum = lv_label_create(obj);
        lv_label_set_text(serNum, SerialNum);
        //lv_obj_set_flex_grow(label, 1);
    }
...
const char * lv_list_get_btn_text_new(lv_obj_t * list, lv_obj_t * btn)
{
    LV_UNUSED(list);
    uint32_t i;
    for(i = 0; i < lv_obj_get_child_cnt(btn); i++) {
        lv_obj_t * child = lv_obj_get_child(btn, i+1);
        if(lv_obj_check_type(child, &lv_label_class)) {
            return lv_label_get_text(child);
        }
